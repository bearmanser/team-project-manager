...
function dedupeOrganizationUsers(projects: ProjectDetail[]): OrganizationUser[] {
...
function App() {
    ...
    const organization = useMemo<OrganizationSummary | null>(() => {
        ...
        return {
            ...
            repoCount: workspace.projects.reduce((count, project) => count + project.repoCount, 0),
            openBugCount: workspace.projects.reduce((count, project) => count + project.openBugCount, 0),
            memberCount: organizationUsers.length || selectedProject?.members.length || 1,
        };
    }, [organizationUsers.length, selectedProject?.members.length, user, workspace]);
    ...
    async function bootstrapWorkspace(): Promise<void> {
        ...
        if (isGitHubCallback) {
            ...
            try {
                ...
                setNotice("GitHub connected. You can now create a project with a single repository.");
            } catch (reason) {
                clearSession();
                setError(getFriendlyError(reason));
            } finally {
    ...
    useEffect(() => {
        if (!token || !workspace || !organizationOpened || selectedProject || organizationSection !== "users") {
            return;
        }
        ...
        void (async () => {
            try {
                ...
                const responses = await Promise.all(workspace.projects.map((project) => getProject(token, project.id)));
                if (cancelled) {
                    return;
                }
                setOrganizationUsers(dedupeOrganizationUsers(responses.map((response) => response.project)));
            } catch (reason) {
                if (!cancelled) {
                    setError(getFriendlyError(reason));
                }
    ...
    }, [organizationOpened, organizationSection, selectedProject, token, workspace]);
    ...
    if (selectedProject) {
        const projectSidebar = (
            <SideNav
                ...
                topSlot={
                    <Stack gap="3">
                        <Text fontSize="xs" textTransform="uppercase" letterSpacing="0.14em" color="#90a0b7">
                            Quick swap project
                        </Text>
                        <select
                        ...
                        <SurfaceCard p="4" bg="#0f141b">
                            <Stack gap="2">
                                <Text color="#d8e1ee">Role: {selectedProject.role}</Text>
                                <Text color="#90a0b7">
                                    {selectedProject.repositories[0]?.fullName ?? "No repository connected"}
                                </Text>
                            </Stack>
                        </SurfaceCard>
                    </Stack>
                    ...
                    <Stack gap="2">
                        ...
                        <Text color="#90a0b7">
                            {user.githubConnected
                                ...
                                : "Connect GitHub to create repository-backed projects"}
                        </Text>
                    </Stack>
                </SurfaceCard>
            }
