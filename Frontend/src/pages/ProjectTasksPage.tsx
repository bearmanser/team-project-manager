          ...
          >
            {PRIORITY_OPTIONS.map((priority) => (
              <option
                key={priority}
                value={priority}
                style={getPriorityOptionStyle(priority)}
              >
  ...
  useEffect(() => {
    return () => {
      const {
        ...
        onCleanupProductBacklogDoneTasks: onCleanup,
      } = cleanupRef.current;
      ...
      const taskIds = currentProject.tasks
        .filter(
          (task) =>
            getTaskPlacement(task, currentProject.activeSprint) === "product" &&
            task.status === "done"
        )
        .map((task) => task.id);
    ...
    ? [
        {
          ...
          description:
            "The active sprint work currently flowing through the board.",
          tasks: sprintBacklog,
        ...
        },
        {
          ...
          description:
            "Ready work that has not been pulled into the current sprint yet.",
    ...
    <Flex direction="column" gap="6" flex="1" minH="0">
      ...
      >
        {sections.map((section) => {
          ...
          const isSprintSection =
            project.useSprints && section.placement === "sprint";
          ...
          return (
            <SurfaceCard
              ...
              bg={
                isDropTarget ? "var(--color-bg-hover)" : "var(--color-bg-card)"
              }
              onDragOver={(event) => {
                if (!project.useSprints) {
                  return;
              ...
              <Stack gap="0" flex="1" minH="0" overflowY="auto" pe="1">
                {section.tasks.length ? (
                  ...
                  <Stack p="6" gap="2">
                    ...
                    <Text color="var(--color-text-muted)">
                      Add a task to start filling this backlog.
                    </Text>
                  </Stack>
                )}
              </Stack>
