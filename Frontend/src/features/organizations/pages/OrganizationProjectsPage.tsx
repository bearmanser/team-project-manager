...
export function OrganizationProjectsPage({
  ...
  const canSubmitProject =
    canCreateProject &&
    Boolean(createProjectForm.name.trim()) &&
    !isCreatingProject;
  ...
  return (
    <Stack gap="6">
      ...
      {githubRepoError ? (
        ...
        >
          <Text fontSize="sm" color="var(--color-danger-text)">
            {githubRepoError}
          </Text>
        </SurfaceCard>
      ) : null}
            ...
            >
              <Stack gap="1" flex="1" minW="260px">
                ...
                <Text color="var(--color-text-subtle)" fontSize="sm">
                  {project.memberCount} people - {project.openBugCount} open
                  bugs - updated {formatShortDate(project.updatedAt)}
                </Text>
              </Stack>
              <Button
                borderRadius="lg"
          ...
          <Stack p="6" gap="2">
            ...
            <Text color="var(--color-text-muted)">
              {canCreateProject
                ...
                : "An owner or admin can add the first project here."}
            </Text>
          </Stack>
        )}
      </SurfaceCard>
            ...
            {isGitHubConnected ? (
              <>
                ...
                <Text color="var(--color-text-muted)" fontSize="sm">
                  Optional. You can connect one repository now or later in
                  project settings.
                </Text>
              </>
            ) : (
              <>
                <Text color="var(--color-text-muted)">
                  GitHub is not connected yet. You can still create the project
                  now and connect a repository later.
                </Text>
                <Button
                  borderRadius="lg"
                  variant="outline"
